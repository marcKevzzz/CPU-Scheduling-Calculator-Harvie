...
export function resetUI(algorithm) {
  ...
  document.getElementById("cpuTotal").classList.add("hide");
...
export function addRow(tableSelector, algorithm = "", isFirstRow = false) {
  ...
  let rowContent = `
    <td class="border border-gray-400 p-1 jobs">
               J${processCounter++}
              </td>
  ...
  if (algorithm === "RR" && isFirstRow) {
    rowContent += `
       ...
       <td class="border border-gray-400 p-1 " class="timeQuantum-col">
                <input
                  class="input border-0 outline-0 w-full bg-transparent drop-shadow-none ps-1 ring-0 input py-2"
  ...
  if (algorithm === "NPP" || algorithm === "PP") {
    rowContent += `
       <td class="border border-gray-400 p-1 " class="priority-col">
                <input
...
export function getProcessData(tableSelector, mode) {
